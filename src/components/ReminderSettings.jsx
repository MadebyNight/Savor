...
export default function ReminderSettings(){
 ...
 return <section className="reminder-settings"><h3>营养周报提醒</h3><p>两种提醒共用时间。前台仅显示应用内提示，后台按系统通知开关发送。查看目标周营养回顾即可停止该周提醒。</p>
  ...
  <label>每周几<select aria-label="每周几" value={draft.weekday} disabled={busy||!status} onChange={e=>setDraft({...draft,weekday:Number(e.target.value)})}>{['一','二','三','四','五','六','日'].map((d,i)=><option key={d} value={i+1}>周{d}</option>)}</select></label>
  <label>提醒时间<input type="time" value={draft.time} disabled={busy||!status} onChange={e=>setDraft({...draft,time:e.target.value})}/></label>
