...
export default function ReminderSettings(){
 ...
 return <section className="reminder-settings">
  ...
  <div className="reminder-schedule"><label>每周几<AppSelect aria-label="每周几" value={draft.weekday} disabled={busy||!status} onChange={e=>setDraft({...draft,weekday:Number(e.target.value)})}>{['一','二','三','四','五','六','日'].map((d,i)=><option key={d} value={i+1}>周{d}</option>)}</AppSelect></label>
  ...
  </div><button className="primary" disabled={busy||!status} onClick={async()=>{setBusy(true);setError('');setMessage('');try{const s=await saveReminders(draft);setStatus(s);setDraft(s.settings);setMessage(draft.system&&!s.settings.system?'系统通知未获许可，未启用；可到系统设置中打开。':'提醒设置已保存');}catch(e){setError(e.message);}finally{setBusy(false);}}}>保存提醒设置</button>
  {status?.nextAt>0&&<p>下一次：{new Date(status.nextAt).toLocaleString('zh-CN',{hour12:false})}</p>}
  {isNative()&&status&&!status.permission&&<p role="status">系统通知未开启</p>}
