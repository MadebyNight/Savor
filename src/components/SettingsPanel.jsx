...
export default function SettingsPanel({state,onRestore,onSyncTarget}) {
  ...
  return <div className="panel settings-panel">
    ...
    <nav className="settings-pages" aria-label="设置分页">
      {[['ai','AI 配置'],['backup','备份恢复'],['sync','坚果云同步']].map(([id,label])=><button key={id} type="button" aria-current={page===id?'page':undefined} aria-controls={`settings-${id}`} onClick={()=>setPage(id)}>{label}</button>)}
    </nav>
