...
async function joinImages(value,get) {
  ...
  if(value && typeof value==='object') {if(value.syncImage)return get(value.syncImage);return Object.fromEntries(await Promise.all(Object.entries(value).map(async([k,v])=>[k,await joinImages(v,get)])));}return value;
}
...
export async function uploadVersion(context,state) {
  ...
  for(const [id,data] of Object.entries(images)){const r=await context.call('images/'+id+'.json','PUT',JSON.stringify(data),{'Content-Type':'application/json','If-None-Match':'*'});if(r.status!==412)assertResponse(r);}
