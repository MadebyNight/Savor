...
with sync_playwright() as p:
    ...
    date.click()
    year = page.get_by_role('textbox', name='年份')
    month = page.get_by_role('textbox', name='月份')
    year.fill('2024')
    year.press('Tab')
    month.fill('2')
    month.press('Tab')
    page.get_by_role('button', name='2024-02-29', exact=True).click()
    page.get_by_role('button', name='确认选择', exact=True).click()
    expect(date).to_have_attribute('value', '2024-02-29')
    date.click()
    year.fill('0000')
    year.press('Tab')
    ...
    browser.close()
print('PASS pickers: month boundaries, year/month jump, commit, cancel, back, Escape, categories, narrow layout, no native controls')
