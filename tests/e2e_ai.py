...
with sync_playwright() as p:
 ...
 page.reload();page.wait_for_load_state('networkidle');open_text();reopen();expect(page.get_by_label('草稿名称1',exact=True)).to_have_value('待补充菜');defer()
 page.get_by_role('button',name='AI 配置',exact=True).click()
 ...
 defer();page.set_viewport_size({'width':390,'height':844});page.get_by_role('button',name='返回',exact=True).click();expect(page.get_by_role('heading',name='设置与数据',exact=True)).not_to_be_visible();page.get_by_role('navigation',name='主导航').get_by_role('button',name='点单',exact=True).click();page.get_by_role('button',name='设置与备份').click();expect(page.get_by_role('heading',name='设置与数据',exact=True).first).to_be_visible();page.screenshot(path=str(OUT/'ai-mobile.png'),full_page=True)
 page.get_by_role('navigation',name='主导航').get_by_role('button',name='冰箱',exact=True).click();expect(page.get_by_role('heading',name='冰箱',exact=True)).to_be_visible();expect(page.get_by_role('button',name='手动添加',exact=True)).to_be_visible();page.screenshot(path=str(OUT/'mobile-home.png'),full_page=True)
