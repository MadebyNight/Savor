...
with sync_playwright() as p:
 ...
 nav('上传菜谱');page.get_by_placeholder('给这道菜起个名字').fill('E2E番茄菜');page.get_by_label('食材名称',exact=True).fill('测试番茄');page.get_by_label('数量',exact=True).fill('100');page.get_by_label('步骤1',exact=True).fill('洗净并炒熟');nav('确认保存到菜品库')
 ...
 file=OUT/'backup.json';download.value.save_as(file);backup=json.loads(file.read_text('utf-8'));assert backup['state']['weeks']['2026-10-05']['0-早'][0]['servings']==3
