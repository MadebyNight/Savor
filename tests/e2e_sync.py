...
with sync_playwright() as p:
 ...
 def upload(page):
  ...
  inspect(page);page.get_by_role('button',name='上传到云端',exact=True).click()
 ...
 b=device();inspect(b);expect(b.get_by_text('本地与云端需要选择版本',exact=True)).to_be_visible();b.get_by_role('button',name='下载到本机',exact=True).click();b.wait_for_function('JSON.parse(localStorage.getItem("shiguang-v1")).recipes.some(r=>r.name==="设备A初始菜")')
 ...
 b.get_by_role('button',name='下载到本机',exact=True).click();b.wait_for_function('JSON.parse(localStorage.getItem("shiguang-v1")).recipes.some(r=>r.name==="A离线修改")')
 ...
 before=data(b);inspect(b);b.get_by_role('button',name='下载到本机',exact=True).click();expect(b.get_by_text('云端数据校验失败，当前数据保留',exact=True)).to_be_visible();assert data(b)==before
