...
with sync_playwright() as p:
    ...
    def section(name):page.get_by_role('navigation',name='设置分页').get_by_role('button',name=name,exact=True).click()
    click('设置与备份')
    expect(page.get_by_role('button',name='识别',exact=True)).to_have_attribute('aria-current','page')
    ...
    page.get_by_label('识别原文',exact=True).fill('测试识别素材')
    ...
    expect(page.get_by_alt_text('待识别图片',exact=True)).to_have_attribute('src',preview)
    section('AI 配置');page.get_by_label('模型',exact=True).fill('unsaved-model')
    section('备份恢复');expect(page.get_by_label('导入备份',exact=True)).to_be_visible()
    section('坚果云同步');page.get_by_label('账号',exact=True).fill('unsaved-account')
    section('识别');expect(page.get_by_label('识别原文',exact=True)).to_have_value('测试识别素材')
    ...
    # 重开设置默认识别；只检查已持久化的图片和素材。
    click('返回');click('设置与备份')
    expect(page.get_by_role('button',name='识别',exact=True)).to_have_attribute('aria-current','page')
    page.reload();page.wait_for_load_state('networkidle');click('设置与备份')
    expect(page.get_by_alt_text('待识别图片',exact=True)).to_have_attribute('src',preview)
    for width,height in [(320,740),(390,844),(740,360),(1280,900)]:
        page.set_viewport_size({'width':width,'height':height})
        for name in ['识别','AI 配置','备份恢复','坚果云同步']:
            section(name)
    ...
    page.set_viewport_size({'width':390,'height':844});section('识别')
    click('移除图片');page.reload();click('设置与备份')
    ...
    browser.close();print('PASS: four settings pages, album/camera routing, cancel/reselect/size/read-error, draft persistence, responsive layout')
