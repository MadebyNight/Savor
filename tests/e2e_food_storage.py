...
with sync_playwright() as p:
 ...
 page.get_by_label('食材名称',exact=True).fill('鸡胸肉');page.get_by_label('保存方式',exact=True).select_option('chilled')
 expect(page.get_by_label('保存天数',exact=True)).to_have_value('1')
 page.get_by_label('保存天数',exact=True).fill('5')
 page.get_by_label('保存方式',exact=True).select_option('frozen');click('取消')
 expect(page.get_by_label('保存方式',exact=True)).to_have_value('chilled');expect(page.get_by_label('保存天数',exact=True)).to_have_value('5')
 page.get_by_label('保存方式',exact=True).select_option('frozen');click('确认重新估算')
 ...
 expect(page.get_by_label('保存方式',exact=True)).to_have_value('frozen')
 page.get_by_label('保存天数',exact=True).fill('10');click('保存食材修改')
 page.get_by_label('期限筛选',exact=True).select_option('unknown');expect(page.locator('.stock-compact-row')).to_have_count(0)
 page.get_by_label('期限筛选',exact=True).select_option('normal');expect(page.locator('.stock-compact-row')).to_have_count(1)
