...
with sync_playwright() as p:
    ...
    manage.click()
    page.get_by_label('分类名称',exact=True).fill('夜宵')
    ...
    page.get_by_label('分类名称',exact=True).fill('家常菜')
    ...
    expect(page.get_by_role('button',name='管理家常菜',exact=True)).to_have_count(1)
    page.get_by_label('菜谱转入分类',exact=True).click()
    page.get_by_role('button',name='夜宵',exact=True).click()
    page.get_by_role('button',name='删除此分类',exact=True).click()
    page.get_by_role('button',name='确认删除分类',exact=True).click()
    page.get_by_role('button',name='关闭弹窗',exact=True).click()
    page.reload(wait_until='networkidle')
    assert page.evaluate('JSON.parse(localStorage.getItem("shiguang-v1")).recipes.every(r=>r.category==="夜宵")')
