...
with sync_playwright() as p:
 ...
 page.get_by_role('button',name='查看营养回顾',exact=True).click();expect(page.get_by_role('region',name='菜单营养回顾')).to_contain_text(target);expect(page.locator('.reminder-banner')).to_have_count(0)
 page.get_by_role('button',name='返回周菜单',exact=True).click();page.reload(wait_until='domcontentloaded');page.get_by_role('navigation',name='主导航').wait_for();expect(page.locator('.reminder-banner')).to_have_count(0)
